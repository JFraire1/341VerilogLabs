...
module DataPath(
    ...
    PC pc(.Din(BranchMuxOut), .clock(clock), .Reset(Reset), .PC_Out(PCaddin));
    ...
    .RegDst(RegDst), .MemRead(MemRead), .MemWrite(MemWrite), .MemtoReg(MemtoReg), .ALUSrc(ALUSrc), .Branch(Branch));
